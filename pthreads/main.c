...
// Логирование каждых k итераций (1 = логировать все)
...
QAPProblem *qap_load_from_file(const char *path)
{
    ...
    for (size_t i = 0; i < n*n; i++)
        if (fscanf(fp, "%lf", &flow[i]) != 1) { free(flow); free(dist); fclose(fp); return NULL; }

    for (size_t i = 0; i < n*n; i++)
        if (fscanf(fp, "%lf", &dist[i]) != 1) { free(flow); free(dist); fclose(fp); return NULL; }
...
void *sa_thread_func(void *arg)
{
    ...
    for (long long iter = 0; iter < a->n_iters; iter++)
    {
        ...
        // LOGGING
        if (a->trace && (iter % LOG_EVERY_ITER == 0))
        {
            a->trace[a->trace_len++] = cur->cost;
        }

        if ((iter % n)==0 && T > 1e-8) T *= a->alpha;
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        fprintf(stderr, "Usage: %s instance.dat [nthreads] [iters] [restart_period] [shake_moves]\n", argv[0]);
        return 1;
    }
    ...
    if (!p) { fprintf(stderr, "Failed to load instance\n"); return 1; }
    ...
    for (int t = 0; t < nthreads; t++)
    {
        ...
        // LOGGING
        long long approx_logs = iters / LOG_EVERY_ITER + 5;
    ...
    {
        char fname[64];
        sprintf(fname, "trace_thread_%d.txt", t);
